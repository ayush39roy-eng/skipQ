                        ...
                        <div className="mx-auto flex w-full max-w-[1400px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:flex-nowrap sm:gap-6 sm:px-6">
                            <div className="flex flex-1 items-center gap-3">
                                <Link href="/" className="flex items-center gap-2">
                                    ...
                                    />
                                    <span className="hidden text-sm font-semibold tracking-tight text-[rgb(var(--text))] sm:inline">Canteen Portal</span>
                                </Link>
                            ...
                            <nav className="flex flex-1 flex-wrap items-center justify-end gap-2 text-xs font-medium sm:justify-end">
                                {session?.role !== 'VENDOR' && <Link className="btn-secondary px-2 py-1.5 sm:px-3" href="/canteens">Canteens</Link>}
                                {session?.role === 'USER' && <Link className="btn-secondary px-2 py-1.5 sm:px-3" href="/order"><span className="sm:hidden">Orders</span><span className="hidden sm:inline">Order History</span></Link>}
                                {session?.role === 'VENDOR' && <Link className="btn-secondary px-2 py-1.5 sm:px-3" href="/vendor">Vendor</Link>}
                                {session?.role === 'ADMIN' && <Link className="btn-secondary px-2 py-1.5 sm:px-3" href="/admin">Admin</Link>}
                                {!session ? (
                                    <Link href="/login" className="btn px-3 py-2 sm:px-4">Login</Link>
                                ) : (
                                    <a href="/api/auth/logout" className="btn px-3 py-2 sm:px-4">Logout</a>
                                )}
                            </nav>
                        </div>
                    </header>
